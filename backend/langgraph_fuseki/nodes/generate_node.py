...
### 6. 기술적 용어 사용 금지 (매우 중요!)
- "수렴노드", "엔티티", "Thread", "BFS 경로" 같은 기술적 용어 절대 사용 금지
- "연결된 인물", "관련된 사건", "중요한 관계" 등 일반인이 이해할 수 있는 표현 사용
- 나쁜 예: "수렴노드가 연결하는 엔티티들"
- 좋은 예: "여러 인물과 사건을 연결하는 중요한 관계"
...
**마크다운 형식으로 작성하되, 예전 구조를 유지하세요:**

### 핵심 답변
질문에 대한 직접적이고 명확한 답변을 **한 문장**으로 먼저 제시하세요.
- 비교 질문: "A가 더 많습니다" 형태로 결론 제시
- 횟수 질문: "총 N번입니다" 형태로 구체적 숫자 제시
- "-입니다" 체로 작성

### 상세 설명
핵심 답변을 바탕으로 **2-3문단**으로 자연스럽게 서술하세요 (200-400자).
- **연도, 사건명, 인물명**을 명확히 언급하고 **굵게** 표시
- 중요한 키워드는 **굵게** 강조
- "-입니다" 체로 작성
...
### 요약
한 문장으로 핵심을 정리하세요 ("-입니다" 체, 한국어만 사용).

**중요: 섹션명("핵심 답변", "상세 설명", "요약")은 출력하지 마세요. 내용만 마크다운 형식으로 작성하세요.**

**예시 출력:**
조선이 대한제국으로 전환된 주요 사건은 **1897년 국호 변경**입니다.

**1896년** **관제 복귀**로 소윤 직제가 다시 도입되며 중앙 권력 구조가 재편되었고, 이어서 **1897년** **국호가 대한제국으로 변경**되면서 황제 체제가 확립되었습니다. 

이 과정에서 **태자 용어의 복고**가 이루어졌으며, **정국공신** 제도가 재정비되어 국가의 권위 체계가 재구성되었습니다.
