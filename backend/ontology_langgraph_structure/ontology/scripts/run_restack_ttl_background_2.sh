...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/restack_ttl_generation_2_$(date +%Y%m%d_%H%M%S).log"

# .restack2 파일 경로
RESTACK_FILE="backend/ontology_langgraph_structure/ontology/instances/.restack2"

echo "=========================================="
echo "🚀 .restack2 TTL 재생성기 백그라운드 실행"
echo "=========================================="
echo ""

# .restack2 파일 확인
if [ ! -f "$RESTACK_FILE" ]; then
    echo "❌ .restack2 파일이 없습니다: $RESTACK_FILE"
    exit 1
fi
...
if [ "$RESTACK_COUNT" -eq 0 ]; then
    echo "⚠️  .restack2 파일에 처리할 행이 없습니다."
    exit 1
fi

echo "📋 .restack2 파일: $RESTACK_FILE"
...
nohup bash -c "source .venv/bin/activate && caffeinate -i python -u backend/ontology_langgraph_structure/ontology/scripts/generate_restack_ttl_2.py" > "$LOG_FILE" 2>&1 &

# PID 저장
PID=$!
echo "🔢 프로세스 ID (PID): $PID"
echo "$PID" > "$LOG_DIR/restack_ttl_generator_2.pid"
