...
"""
LangGraph(app_ver2 / langgraph_structure2) -> 답변 생성 -> RAGAS 평가
...
async def answer_with_appver2(question: str, tag: str = "chat") -> Dict[str, Any]:
    """
    app_ver2가 내부에서 하는 것처럼 create_graph_flow()를 만들어 ainvoke로 실행.

    ✅ 여기서 채움
    - gen_elapsed_sec: perf_counter로 강제 측정
    - llm_meta_answer/total: 없으면 시간/토큰 추정치로 채움
    """
    from backend.langgraph_structure2.graph import create_graph_flow  # lazy import

    app = create_graph_flow()
    initial_state = {"query": question, "tag": tag}
...
def main():
    ...
    ap.add_argument("--outdir", default=str(PROJECT_ROOT / "backend" / "ragas" / "appver2" / "runs"))
    ap.add_argument("--run-name", default="appver2_results_only")
    ...
    for it in items:
        ...
        try:
            # 1) 답변 생성 (LangGraph)
            result = asyncio.run(answer_with_appver2(question, tag=args.tag))
