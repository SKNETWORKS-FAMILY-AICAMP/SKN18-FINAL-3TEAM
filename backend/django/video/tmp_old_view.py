...
# 동영상을 유니티에서 읽을 수 있게 해주는 FFmpeg 관련 라이브러리 임포트
# 나중에 AWS에서도 서버 자체에 ffmpeg 깔려 있어야 함(이거 requiremets.txt로 해결 안됩니다.)
...
async def generate_scenario(request):
    ...
    try:
        ...
        # 👇 [여기서부터 수정!] 👇
        # ======================================================
        # [Fix] 배경 우선순위 로직: MP4(영상) > PNG(이미지)
        # ======================================================
        
        # 1. 디버깅: 도대체 LangGraph가 뭘 뱉었는지 눈으로 확인
        print(f"🔍 [Debug] LangGraph 결과 키 목록: {list(result.keys())}")
        
        # 2. 후보군 수색 (여러 가지 키 이름을 다 뒤짐)
        # (주의: video_url, background_video_url 등 노드에서 쓰는 키를 다 넣음)
        candidate_video = (
            result.get('video_url') or 
            result.get('background_video_url') or 
            result.get('video_path') or
            final_script.get('background_video_url')
        )
        
        candidate_image = (
            result.get('background_url') or 
            result.get('background_image_url') or
            final_script.get('background_url')
        )

        target_url = ""

        # 3. [핵심] 우선순위 결정 (MP4가 있으면 무조건 그걸 쓴다!)
        if candidate_video and ".mp4" in str(candidate_video):
            target_url = candidate_video
            print(f"🎬 [Select] 성공! MP4 영상을 배경으로 선택함: {target_url}")
        elif candidate_image:
            target_url = candidate_image
            print(f"🖼️ [Select] 영상 없음. PNG 이미지를 배경으로 선택함: {target_url}")
        else:
            print("⚠️ [Warning] 배경으로 쓸만한 파일(MP4/PNG)을 찾지 못했습니다.")

        # 4. JSON에 강제 주입
        if target_url and 'scenes' in final_script:
            print(f"🔧 [Fix] 유니티 Location 덮어쓰기 적용 -> {target_url}")
            final_script['background_url'] = target_url  # 전역 배경도 수정
            for scene in final_script['scenes']:
                scene['location'] = target_url # 장면별 배경 수정
...
class PendingScriptView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            # 👇 [여기서부터 추가/수정된 부분] 주소 세탁 로직 시작! 👇
            # ---------------------------------------------------------
            ngrok_url = "https://tara-multiflorous-frowsily.ngrok-free.dev" # 님 주소

            # JSON 데이터를 문자열로 변환 (모든 URL을 한 번에 찾기 위해)
            json_str = json.dumps(data, ensure_ascii=False)

            # 로컬 주소가 있다면 Ngrok 주소로 일괄 변경
            if "127.0.0.1:8000" in json_str:
                json_str = json_str.replace("http://127.0.0.1:8000", ngrok_url)
            
            if "localhost:8000" in json_str:
                json_str = json_str.replace("http://localhost:8000", ngrok_url)
